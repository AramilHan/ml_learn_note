...
@Author  : -
@Time    : 2025/3/3 23:04
@File    : dataloader4ml100kOneHot.py
@Description : 
"""
import os
import sys
...
def __read_rating_data(path):
    ...
    with open(path, 'r') as f:
        for line in f.readlines():
            d = line.strip().split('\t')
            data_set[(int(d[0]), int(d[1]))] = [get1or0(int(d[2]))]
    return data_set
...
def read_data_set(user_dict, item_dict, path):
    X, Y = [], []
...
def read_data():
    ...
    x_train, y_train = read_data_set(user_dict, item_dict, train_path)
    x_test, y_test = read_data_set(user_dict, item_dict, test_path)
    return x_train, x_test, y_train, y_test


class DataIter:
    def __init__(self, train_x, train_y):
        self.train_set = self.__getMixTrainSet(train_x, train_y)

    def __getMixTrainSet(self, train_x, train_y):
        train_set = [[list(x), y] for x, y in zip(train_x, train_y)]
        return train_set

    def iter(self, batch_size):
        for i in range(len(self.train_set) // batch_size):
            data_set = random.sample(self.train_set, batch_size)
            yield data_set
