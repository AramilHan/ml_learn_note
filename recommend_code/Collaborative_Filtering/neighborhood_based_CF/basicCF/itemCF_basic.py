...
"""
@Author  : -
@Time    : 2025/2/17 21:26
@File    : itemCF_basic.py
@Description :
"""
import os
import sys
...
def trainItemCF(item_users, sim_method, user_items, k=5):
    ...
    recommendations = get_recommendations_by_itemCF(item_sims, user_items)
    return recommendations

def main():
    _, _, train_set, test_set = dataloader.readRecData(fp.Ml_100K.RATING, test_ratio=0.1)
    item_users, user_items = getSet(train_set)
    recommendations_by_itemCF = trainItemCF(item_users, b_sim.cos4set, user_items, k=5)
