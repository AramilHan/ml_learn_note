...
class DeepFM(Model):
    def __init__(self, n_features, user_df, item_df, dim=128):
        ...
        # 初始化MLP层
        self.mlp_layer = self.__mlp(dim * total_feature_num)

    def __mlp(self, dim):
        return Sequential([
            layers.Dense(dim // 2, activation='relu'),
            layers.Dense(dim // 4, activation='relu'),
            layers.Dense(dim // 8, activation='relu'),
    ...
    def deep(self, feature_embs):
        ...
        feature_embs = tf.reshape(feature_embs,(feature_embs.shape[0], -1))
        # [batch_size, 1]
    ...
    def call(self, inputs):
        ...
        fm_output = self.__fm_cross(all_feature_embs)
        deep_output = self.deep(all_feature_embs)
        out = tf.sigmoid(fm_output + deep_output)
        return out
...
def train(epochs=20, batch_size=1024, lr=0.02, dim=128, eva_per_epochs=1, need_eva=True):
    ...
    model = DeepFM(n_features, user_df, item_df, dim)
