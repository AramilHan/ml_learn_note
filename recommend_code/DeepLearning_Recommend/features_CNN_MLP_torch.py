...
"""
@Author  : -
@Time    : 2025/3/10 16:42
@File    : features_CNN_MLP_torch.py
@Description : 
"""
import os
import sys
...
class featuresCNN(nn.Module):
    def __init__(self, n_user_features, n_item_features, user_df, item_df, dim):
        super(featuresCNN, self).__init__()
        # 随机初始化所有特征的特征向量
        ...
        # 定义CNN
        self.Conv = nn.Conv1d(in_channels=total_features_num, out_channels=1, kernel_size=3)
    ...
    def forward(self, u, i, is_train=True):
        user_ids = torch.LongTensor(self.user_df.loc[u].values)
        item_ids = torch.LongTensor(self.item_df.loc[u].values)
        ...
        # [batch_size]
        uv = torch.squeeze(uv)
        return self.sigmoid(uv)

def doEva(net, test_triples):
    d = torch.LongTensor(test_triples)
    u, i, r = d[:, 0], d[:, 1], d[:, 2]
    with torch.no_grad():
        out = net(u, i, is_train=False)
...
def train(epochs=10, batch_size=1024, lr=0.001, dim=256, eva_per_epochs=1):
    ...
    for e in range(epochs):
        ...
        for u, i, r in DataLoader(train_triples, batch_size=batch_size, shuffle=True):
            ...
            loss.backward()
            optimizer.step()
        print(f"Epoch: {e}  |  Loss: {all_loss/(len(train_triples) // batch_size):.4f}")
